<form action="{{ $action }}" method="POST" x-data="{
    ...
    <div class="space-y-4">
        ...
        {{-- Kontainer Toggle dan Input Tanggal --}}
        <div class="p-4 border rounded-lg dark:border-gray-600">
            ...
            <div>
                <div class="flex items-center space-x-2 transition" :class="{ 'opacity-50 cursor-not-allowed': !enableDate }">
                    ...
                    <button type="button" @click="$refs.dateInput.showPicker()" :disabled="!enableDate" class="p-2.5 border rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600">
    ...
    <div class="mt-6 flex justify-end space-x-3">
        @if($method === 'PUT')
            <a href="{{ route('rencana.show', $rencana) }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:bg-gray-600">
                Batal
            </a>
        @else
            <a href="{{ route('rencana.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:bg-gray-600">
                Batal
            </a>
        @endif
        <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
            {{ $method === 'PUT' ? 'Update Rencana' : 'Simpan Rencana' }}
        </button>
    </div>
</form>
